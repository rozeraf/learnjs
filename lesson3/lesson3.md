...
**Цель:** Научиться работать с массивами в TypeScript: создавать, изменять,
перебирать и использовать в функциях.
...
5. **Перебор массива**
   ...
   - (кратко упомянуть `.map`, чтобы знать, что есть ещё более "функциональный"
     стиль).
...
4. **Дополнительно (по желанию)**
   - Написать функцию `greetAll(names: string[])`, которая возвращает **новый
     массив приветствий** (используя `.map`).
   - Это закрепит разницу между `.forEach()` (ничего не возвращает) и `.map()`
